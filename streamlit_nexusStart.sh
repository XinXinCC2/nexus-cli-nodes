...
# 设置 screen 命令的路径
SCREEN_CMD="./screen_local/bin/screen"
...
handle_sessions() {
    while true; do
        ...
        echo "=========================================="
        sessions=$($SCREEN_CMD -ls | grep "nexu_" | awk '{print $1}')
...
start_all_nodes() {
    ...
    # 检查本地 screen 是否存在
    if [ ! -f "$SCREEN_CMD" ]; then
        echo "错误：未找到本地 screen 程序，请确保 screen_local/bin/screen 存在"
        read -p "按回车键继续..."
        return
    fi
    ...
    while IFS= read -r node_id || [ -n "$node_id" ]; do
        ...
        # 在会话中启动服务
        $SCREEN_CMD -S "$session_name" -X stuff "./nexus-network start --node-id $node_id$(printf '\r')"
        if [ $? -ne 0 ]; then
    ...
    echo "可以使用 '$SCREEN_CMD -ls' 查看所有会话"
    echo "使用 '$SCREEN_CMD -r 会话名' 进入特定会话查看详情"
    read -p "按回车键继续..."
}
...
stop_all_nodes() {
    ...
    if [ -z "$sessions" ]; then
        ...
        for session in $sessions; do
            ...
            # 发送 q 命令停止服务
            $SCREEN_CMD -S "$session" -X stuff "q$(printf '\r')"
            # 等待服务停止
            sleep 2
            # 关闭会话
            $SCREEN_CMD -S "$session" -X quit
            if [ $? -eq 0 ]; then
                echo "成功关闭会话: $session"
            else
                echo "关闭会话失败: $session"
