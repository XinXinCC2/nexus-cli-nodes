...
view_all_logs() {
    ...
    # 创建临时日志目录
    TEMP_LOG_DIR="/tmp/nexus_logs_$$"
    mkdir -p "$TEMP_LOG_DIR"
    
    # 遍历所有会话
    for session in $sessions; do
        # 从会话名中提取 node_id
        node_id=$(echo "$session" | sed 's/nexu_//')
        temp_log_file="$TEMP_LOG_DIR/nexus_${node_id}.log"
        
        # 获取会话的日志并保存到临时文件
        screen -S "$session" -X hardcopy "$temp_log_file"
        
        # 获取第5行日志
        fifth_line=$(sed -n '5p' "$temp_log_file" 2>/dev/null)
